...
export default function PayrollPage() {
  ...
  const [lateAfterTime, setLateAfterTime] = useState("08:00"); const [lateTimeDraft, setLateTimeDraft] = useState("08:00"); const [savingRules, setSavingRules] = useState(false);
  async function load() { setLoading(true); try { const data = await api<{ role: string; staff: Staff[]; entries: Entry[]; bursarAccess: boolean; payrollRules: { lateAfterTime: string } }>(`payroll?month=${month}`); setRole(data.role); setStaff(data.staff); setEntries(data.entries); setBursarAccess(data.bursarAccess); setLateAfterTime(data.payrollRules.lateAfterTime); setLateTimeDraft(data.payrollRules.lateAfterTime); } catch (e) { setError((e as Error).message); } finally { setLoading(false); } }
  ...
  async function saveLateAfterTime() { setSavingRules(true); try { await api("payroll/setPayrollRules", { method: "POST", body: { lateAfterTime: lateTimeDraft } }); setLateAfterTime(lateTimeDraft); } catch (e) { alert((e as Error).message); } finally { setSavingRules(false); } }
  ...
  return <div><PageHeader title="Payroll" subtitle="Monthly salaries, late-coming penalties, rewards and deductions." actions={<Button onClick={generate}>Generate {month} payroll</Button>} />
    ...
    {canManagePayroll && <Card title="Attendance rules" style={{ marginBottom: 16 }}>
      <p>Late days are pulled automatically from real clock-in records when payroll is generated — a staff member counts as late on any day they clock in after this time. A day they never clock in at all is an absence, not lateness, and never counts here.</p>
      <div style={{ display: "flex", alignItems: "flex-end", gap: 10 }}>
        ...
        <Button onClick={saveLateAfterTime} loading={savingRules} disabled={lateTimeDraft === lateAfterTime}>Save</Button>
      </div>
    </Card>}
