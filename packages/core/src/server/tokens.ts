...
import {
  ...
  type GateClaims,
  type ApplicationTestClaims,
  type GameInviteClaims,
} from "../auth";

// All JWT signing/verification lives here, server-only (the `server-only`
// import above makes any accidental client-bundle import a build-time
// error) — split out from auth.ts so the signing secret and its access
// function can never end up in a browser bundle, however auth.ts itself
// evolves later. See auth.ts for the client-safe cookie/claims exports.
export { COOKIE_NAMES };
